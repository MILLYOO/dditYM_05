...
@Controller
public class DepartmentManageUpdateController {
	@Inject
	...
	@PostMapping(value="/info1/departmentUpdate.do", produces=MediaType.APPLICATION_JSON_UTF8_VALUE)
	@ResponseBody
	public Map<String, Object> DepartmentManageUpdate(			
			@Valid @ModelAttribute DepartmentVO department,
			Errors errors
			) {
		hubWebValidateUtil valid = new hubWebValidateUtilImpl(errors, context);
		if(!valid.checkError()) {
			valid.setResultMap(service.updateDepartmentManage(department));		
		}
