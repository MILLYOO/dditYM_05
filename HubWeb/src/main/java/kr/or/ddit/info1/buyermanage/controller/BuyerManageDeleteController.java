 ...
 */
@Controller
public class BuyerManageDeleteController {
	@Inject
	...
	@RequestMapping(value = "/info1/buyerDelete.do", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	@ResponseBody
	public Map<String, Object> buyerDelete(@Validated(DeleteGroup.class) @ModelAttribute BuyerVO buyer, Errors errors) {
		hubWebValidateUtil valid = new hubWebValidateUtilImpl(errors, context);
		if(!valid.checkError()) {
			valid.setResultMap(service.deleteBuyer(buyer));
		}
